...
from .forms import NewCandidateForm, NewJobForm, SubmitCandidateForm, InterviewAddForm, CandidateSearchForm, JobSearchForm
import datetime
...
def recruiter(request, user_username):
	template_name = 'ats/recruiter.html'
...
def logout_user(request):
	logout(request)
	#TODO logout page?
	return render(request, 'ats/index.html')
...
def edit_candidate(request, candidate_pk):
	...
	context = {
		'form': NewCandidateForm(instance=c, initial={
								'tags_languages' : get_candidate_tag_id_by_tag_type(c, 'LANG'),
								'tags_function' : get_candidate_tag_id_by_tag_type(c, 'FUNC'),
								'tags_database' : get_candidate_tag_id_by_tag_type(c, 'DB'),
								'tags_technology' : get_candidate_tag_id_by_tag_type(c, 'TECH'),
								'tags_miscellaneous' : get_candidate_tag_id_by_tag_type(c, 'MISC')
													})
	}
	return render(request, 'ats/edit_candidate.html', context)
...
def get_candidate_tag_id_by_tag_type(candidate, tag_type):
	#accepts: candidate, tag_type
	#returns: list of tag.id for which the candidate is tagged
	tags = [tag.id for tag in candidate.tags.filter(tagtype = TagType.objects.filter(name = tag_type))]
	return tags
